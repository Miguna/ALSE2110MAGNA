...
bool Polinomio::redefinir(std::string pol) {
    ...
    while( posT2 != string::npos ){  //mientras no llegue al final del texto (caracter especial)
        // Encontrando cada termino
        posT2 = pol.find_first_of("+-", posT2 + 1 ); //aca busco el primer caracter ya sea el + o el - lo busco desde el segundo caracter

        //cout << posT1 << " " << posT2 << endl;
        term = pol.substr( posT1, posT2 - posT1 ); //saca el texto entre las lineas post1 y post2
        posT1 = posT2;
        //cout << term << endl;
        ...
        if( vardefinida == false ){
            //cout << "variable" << endl;
            found = term.find_first_not_of("+-1234567890.,"); // encuentreme el nombre de la variable, encuentre el primer termino que no sea + o - o ........
...
bool Polinomio::ordenar(){
    ...
    bool cambio = false;
    do{
        ptr1 = _polCabeza;    //cargo en prt1 lo que hay en Cabeza
        cambio = false;
        while( ptr1->getSiguiente() != NULL ){  //mientras que el siguiente del ptr1 no sea el nulo
            ptr2 = ptr1->getSiguiente();        //asignele al segundo puntero el siguiente de la cabeza
            if( ptr1->getP() < ptr2->getP() ){ // Es necesario intercambiar, esta es una oregunta por la pontencia del monomio
                //cout << ptr1 << " " << ptr2 <<endl;
                cambio = true;
                ptr1->setSiguiente( ptr2->getSiguiente() );  // estas dos instrucciones son las que me hacen avanzar
                ptr2->setSiguiente( ptr1 );                  // en el ordenamiento del polinomio
                //cout << ptr1 << " " << ptr2 <<endl;
                if( _polCabeza == ptr1 ) {
                    _polCabeza = ptr2;
                }else{
